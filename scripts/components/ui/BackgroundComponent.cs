...
public partial class BackgroundComponent : Control {
	...
	/// <param name="forceSolidMode">Whether to switch to solid color mode</param>
	public void SetBackgroundColor(Color newColor, bool forceSolidMode = false) {
	...
	/// <summary>
	/// Updates the opacity at runtime.
	/// </summary>
	/// <param name="newOpacity">New opacity value (0.0 to 1.0)</param>
	public void SetOpacity(float newOpacity) {
		Opacity = Mathf.Clamp(newOpacity, 0.0f, 1.0f);
		ApplySettings();
	}

	/// <summary>
	/// Toggles blur effect at runtime.
	/// </summary>
	...
	/// <summary>
	/// Animates the opacity change over time.
	/// </summary>
	/// <param name="targetOpacity">Target opacity value</param>
	/// <param name="duration">Animation duration in seconds</param>
	/// <returns>The created Tween for further control</returns>
	public Tween AnimateOpacity(float targetOpacity, float duration = 0.5f) {
		var tween = CreateTween();
		tween.TweenMethod(Callable.From<float>(SetOpacity), Opacity, targetOpacity, duration);
		return tween;
	}

	/// <summary>
	/// Fades the background in from transparent.
	/// </summary>
	...
	public Tween FadeIn(float duration = 0.5f) {
		...
		SetOpacity(0.0f);
		return AnimateOpacity(originalOpacity, duration);
	}

	/// <summary>
	/// Fades the background out to transparent.
	/// </summary>
	/// <param name="duration">Fade duration in seconds</param>
	/// <returns>The created Tween for further control</returns>
	public Tween FadeOut(float duration = 0.5f) {
		return AnimateOpacity(0.0f, duration);
	}

	/// <summary>
	/// Updates all settings and recreates the background.
	/// Call this after changing multiple Export properties at runtime.
	/// </summary>
