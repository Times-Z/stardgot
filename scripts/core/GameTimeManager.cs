...
public partial class GameTimeManager : Node {
	...
	public override void _Process(double delta) {
		...
		if (nowNight != _isNight) {
			_isNight = nowNight;
			GetTree().Root.GetNode<NightFilterComponent>
				("GameRoot/ViewportContainer/Viewport/UiLayer/NightFilterComponent")
				.SetNight(_isNight);
		}
	}

	/// <summary>
	/// Gets the formatted time string for display in the UI, including the current time in HH:MM format.
	/// </summary>
	public string GetFormattedTime() => $"{Hours:D2}:{Minutes:D2}";

	/// <summary>
	/// Determines if the current time is considered "night" based on the game time.
	/// Night is defined as any time before 6 AM or after 6 PM.
	/// </summary>
	public bool IsNight() {
